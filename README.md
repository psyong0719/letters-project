...
<style>
  ...
  *{box-sizing:border-box}
  ...
  h1{
    ...
    color:#f2ead9; text-shadow:0 0 12px rgba(241,208,138,.28);
  }
  .hint{ font-size:15px; color:var(--muted); }

  /* Placeholder that reserves broom's spot between title and hint */
  ...
  .line{font-size:22px; line-height:2.1; word-wrap:break-word}

  .ch{
    display:inline-block; white-space:pre;
    margin-right:1.05em; opacity:1; will-change:opacity,transform,filter;
    animation: floatXY var(--t,4.6s) ease-in-out infinite;
  }
  .fade{ animation: inkFade 3.8s ease forwards, floatXY var(--t,4.6s) ease-in-out infinite; }
  ...
  /* Broom sprite (fixed, moves on arrows; snaps back to home spot) */
  .broom{
    position:fixed; width:190px; height:28px; transform:translate(-50%,0) rotate(0deg);
  ...
  .card h2{margin:6px 0 10px;font-size:18px;font-family:"Cinzel", serif}
  ...
  .btn{appearance:none;border:1px solid #223349;background:#0f1725;color:#d5e7fb;padding:8px 12px;border-radius:10px;font-size:12px;cursor:pointer}
...
<!-- Broom (fixed-position sprite that snaps to the home placeholder) -->
...
<div id="modal" class="modal" role="dialog" aria-modal="true" aria-label="Spell list">
  <div class="card">
    ...
    <div style="text-align:right;margin-top:10px"><button id="closeModal" class="btn" type="button">Close</button></div>
  </div>
</div>
...
const broomInner = broom.querySelector('.broomInner');
...
function insertItem(it){
  ...
  if(it.type==='ch'){
    ...
    setFloat(s, revealed?1.2:1);
    s.textContent=it.ch; it.el=s; line.insertBefore(s, caret);
  }else if(it.type==='space'){
    const s=document.createElement('span'); s.className='ch';
    if(!revealed) s.classList.add('fade');
    s.textContent=' '; setFloat(s, revealed?1.2:1); it.el=s; line.insertBefore(s, caret);
  }else if(it.type==='br'){
...
function backspace(){
  for(let i=items.length-1;i>=0;i--){
    const it=items[i];
    if(it?.el){ it.el.remove(); items.splice(i,1); break; }
  }
  moveCaret(); sfx('back');
}

/* ===== Input (English only basic) ===== */
function isAsciiLetter(k){ return /^[A-Za-z]$/.test(k); }
function isPrintable(k){ return k===' ' || isAsciiLetter(k) || ['.',',',"'",'-',':',';'].includes(k); }
...
window.addEventListener('keydown', (e)=>{
  ...
  if(e.key==='Tab'){ e.preventDefault(); if(e.repeat) return; modal.style.display = (modal.style.display==='grid') ? 'none' : 'grid'; if(modal.style.display==='grid') buildSpellList(); return; }
...
function toggleReveal(){
  ...
  if(!revealed){
    ...
    setTimeout(()=>{ els.forEach(el=> el.classList.remove('revealing')); revealed = true; isToggling=false; }, REVEAL_MS+20);
  }else{
    els.forEach(el=>{
      el.classList.remove('revealing');
      el.classList.add('hiding');
    });
    setTimeout(()=>{ els.forEach(el=>{ el.classList.remove('hiding'); el.classList.add('hidden'); }); revealed = false; isToggling=false; }, HIDE_MS+20);
  }
}
  ...
  if(/[A-Za-z]/.test(ch)){
    ...
    for(const p of PATTERNS){
      if(stream.endsWith(p)){
        ...
        setTimeout(()=> eraseSpans(spans, idxs), dur);
...
function eraseSpans(spans, idxs){
  spans.forEach(el=>{ if(!el) return; el.style.transition='opacity .22s ease'; el.style.opacity='0'; setTimeout(()=>el.remove(), 260); });
...
const DUR = {
  'lumos':5200, 'nox':5200, 'expelliarmus':620, 'wingardium leviosa':1100,
  'obliviate':2600, 'expecto patronum':1100, 'imperio':1000, 'crucio':520, 'avada kedavra':0
};
function triggerSpell(name, targetSpans=[]){
  switch(name){
    case 'lumos':{
      const d=DUR[name];
      pad.animate([{filter:'brightness(4.8) saturate(1.06)'},{filter:'brightness(1)'}],{duration:d,fill:'none',easing:'ease-out'});
      sfx('lumos'); return d;
    }
    case 'nox':{
      const d=DUR[name];
      pad.animate([{filter:'brightness(.02) saturate(.5)'},{filter:'brightness(1)'}],{duration:d,fill:'none',easing:'ease-in-out'});
      sfx('nox'); return d;
    }
    case 'expelliarmus':{
      const word = targetSpans.length ? targetSpans : collectLastWord();
      const p=caretPos();
      beam(p.x, p.y, 460, 0, 'rgba(255,130,130,.95)', 520);
      word.forEach((el,i)=>{
        const dx=(Math.random()*2-1)*150, dy=-(80+Math.random()*110), rot=(Math.random()*2-1)*300;
        el.animate(
          [
          ...
          {duration:560,easing:'cubic-bezier(.2,.8,.2,1)',delay:i*10}
        );
      });
      sfx('expel'); return DUR[name];
    }
    case 'wingardium leviosa':{
      const t = Array.from(document.querySelectorAll('.ch')).slice(-50);
      t.forEach((el,i)=>{
        el.animate(
          [
            {transform:'translateY(0px)', composite:'add'},
            {transform:`translateY(-${30+Math.random()*50}px)`, composite:'add'},
            {transform:'translateY(0px)', composite:'add'}
          ],
          {duration:820+Math.random()*460,fill:'none',easing:'ease-in-out',delay:i*12}
        );
      });
    ...
    case 'obliviate':{
      ...
      chars.forEach((el, i)=>{
        ...
        const delay = t * d;
        setTimeout(()=>{
          el.style.transition = 'opacity 1.0s ease, filter 1.0s ease';
    ...
    case 'imperio':{
      const d=DUR[name];
      const t=setInterval(()=>{ insertItem({type:'ch', ch: 'abcdefghijklmnopqrstuvwxyz'[Math.floor(Math.random()*26)]}); }, 95);
      setTimeout(()=> clearInterval(t), d-60);
    ...
    case 'crucio':{
      ...
      f.animate([{opacity:.9},{opacity:0}],{duration:220,fill:'forwards'}); setTimeout(()=>f.remove(),240);
      sfx('crucio'); return d;
    }
    case 'avada kedavra':{
      ...
      g.animate([{opacity:1},{opacity:0}],{duration:600,fill:'forwards'}); setTimeout(()=>g.remove(),620);
...
/* ===== Broom — home is the placeholder between title and hint ===== */
...
function positionBroom(x, y, ang=0){
  broom.style.left = x + 'px';
  broom.style.top  = y + 'px';
  const dir = Math.cos(ang || 0) >= 0 ? 1 : -1;
  broom.style.transform = `translate(-50%,0) rotate(${ang||0}rad)`;
  broomInner.style.transform = `scaleX(${dir})`;
}
function snapBroomHome(){
  const h = homeAnchor();
  br.x=h.x; br.y=h.y; br.vx=0; br.vy=0; br.running=false;
  positionBroom(br.x, br.y, 0);
}
...
function loopB(){
  ...
  const ang = Math.atan2(br.vy, br.vx) || 0;
  positionBroom(br.x, br.y, ang);
...
function sfx(name){
  ...
  switch(name){
    ...
    case 'expel': noise(0.16,'highpass',1900,1.0,0.30); break;
    ...
    case 'imperio': tone(430,0.06,'square',0.18); break;
